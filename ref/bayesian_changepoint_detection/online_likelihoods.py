...
class NormalKnownPrecision(BaseLikelihood):
    ...
    def update_theta(self, data: np.array, t, **kwargs):
        offsets = np.arange(
            1, t + 2
        )  # To calculate new mean params at each node of the next time step,  we need to multiply each of the previous mean value in self.mu array (2u1+data/3, 3u2+data/4 etc.,), we have offsets
