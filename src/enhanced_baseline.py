...
class RenderTSExtrapolator:
    def __init__(
        self, first_frame_sts, first_frame_no, frame_interval, mode=0
    ):  # 0 for naive mode, 1 for Kalman filter
        self.first_frame_sts = first_frame_sts
        ...
        self.updated_frame_interval = frame_interval

        if self.mode == 1:
            self.w = [1, 0]
            self.p = [[1, 0], [0, 1e3]]

    def update(self, frame_sts, frame_no):
        if self.mode == 0:
            pass
        elif self.mode == 1:
            frame_interval_diff = (frame_sts - self.first_frame_sts) / (
            ...
            if frame_no <= self.first_frame_no:
                return
            cur_frame_interval = (frame_sts - self.first_frame_sts) / (
                frame_no - self.first_frame_no
            )
            self.frame_interval = 0.99 * self.frame_interval + 0.01 * cur_frame_interval
            self.first_frame_sts = frame_sts
            self.first_frame_no = frame_no
        else:
            raise NotImplementedError

    def reset(self, first_frame_sts, first_frame_no):
        if self.mode == 0:
            pass
        elif self.mode == 1:
            self.w = [1, 0]
    ...
    def predict_render_ts(self, frame_sts, frame_no):
        if self.mode == 0:
            return frame_sts
        elif self.mode == 1:
            cur_frame_interval = (frame_sts - self.first_frame_sts) / (
            ...
            self.updated_frame_interval = (self.frame_interval - self.w[1]) / self.w[0]
            return self.first_frame_sts + (frame_no - self.first_frame_no) * (
                (self.frame_interval - self.w[1]) / self.w[0]
            )
        elif self.mode == 2:
            self.updated_frame_interval = self.frame_interval
            return self.first_frame_sts + self.frame_interval * (
                frame_no - self.first_frame_no
            )
        else:
            raise NotImplementedError
...
def cal_valid_frame(
    ...
    data = trace_loader(file_path, start_idx=0, len_limit=sim_data_len)  # sim for 20min
...
def cal_single_para_result(
    ...
    expect_display_slot = np.zeros(data.shape[0])
    expect_nearest_display_slot = np.zeros(data.shape[0])
    predicted_render_ts = np.zeros(data.shape[0])
    updated_frame_interval = np.zeros(data.shape[0])
    expect_recv_ts = np.zeros(data.shape[0])
    expect_proc_time = np.zeros(data.shape[0])
    expect_frame_ready_ts = np.zeros(data.shape[0])
    actual_display_slot = np.zeros(data.shape[0])
    ...
    tear_flag = np.zeros(data.shape[0])
    display_ts_residuals = np.zeros(data.shape[0])
    ...
    action_sample = np.zeros(data.shape[0])
    lowest_free_slot = np.zeros(data.shape[0])
    ...
    expect_nearest_display_slot[0] = nearest_display_slot[0]
    expect_proc_time[0] = proc_time_estimator.get_proc_time_estimate()
    predicted_render_ts[0] = data[0, 12]
    updated_frame_interval[0] = frame_interval
    cur_valid_flag[0] = 1
    slot_moved_flag[0] = 1

    render_ts_estimator = RenderTSExtrapolator(
        first_frame_sts=data[0, 12],
        first_frame_no=data[0, 0],
        frame_interval=frame_interval,
        mode=render_ts_estimator_mode,
    )
    recv_time_estimator = TimestampExtrapolator(
        first_frame_sts=data[0, 12],
        first_frame_recv_ts=0,
    ...
    display_ts_residual = (
        invoke_present_ts[0] - nearest_display_ts[0]
    ) % frame_interval
    display_ts_residuals[0] = display_ts_residual
    if (
        display_ts_residual > vsync_slot_threshold
        and display_ts_residual < frame_interval - vsync_slot_threshold
    ):
        tear_flag[0] = 1
    elif display_ts_residual >= 2 and display_ts_residual <= vsync_slot_threshold:
        tear_flag[0] = 2
    ...
    def frame_display_controller(idx, buf_size=0):
        ...
        # can be calculated on the proxy in advance, no need to be calcuated in the client SDK
        cur_frame_sts = render_ts_estimator.predict_render_ts(
            data[idx, 12], data[idx, 0]
        )
        predicted_render_ts[idx] = cur_frame_sts
        updated_frame_interval[idx] = render_ts_estimator.updated_frame_interval
        expect_recv_ts[idx] = recv_time_estimator.extrapolate_local_time(cur_frame_sts)
        expect_proc_time[idx] = proc_time_estimator.get_proc_time_estimate()
        expect_frame_ready_ts[idx] = expect_recv_ts[idx] + expect_proc_time[idx]
        ...
        if render_ctrlable:
            ...
            display_ts_residual = (
                invoke_present_ts[idx] - nearest_display_ts[idx]
            ) % frame_interval
            display_ts_residuals[idx] = display_ts_residual
            if (
                display_ts_residual > vsync_slot_threshold
                and display_ts_residual < frame_interval - vsync_slot_threshold
            ):
                ...
                display_ts_residual >= 2 and display_ts_residual <= vsync_slot_threshold
            ):
                tear_flag[idx] = 2
    ...
    for idx in range(1, data.shape[0]):
        ...
        if idx >= 3:
            render_ts_estimator.update(data[idx - 2, 12], data[idx - 2, 0])
            recv_time_estimator.update(data[idx - 2, 12], data[idx - 2, 5])
            if render_ctrlable:
                proc_time_estimator.update(
                    data[idx, 9], data[idx, 11], data[idx, 6:12].sum()
    ...
    if print_log:
        path = (
            file_path[:-4].replace("data", "detailed_log/baseline_alltear_buf2_normal")
            + "_sim.csv"
        )
    ...
    data[-1, 11] = -1
    valid_idx = np.where(data[:, 11] != -1)[0]
    opt_valid_no = np.unique(actual_display_slot[valid_idx]).size
    opt_fps = opt_valid_no / tot_time

    # per_second_tear = np.logical_and.reduce((data[:, 11] != -1, tear_flag == 2))[:data.shape[0]//60*60]
    per_second_tear = np.logical_and.reduce(
        (data[:, 11] != -1, tear_flag == 2, cur_valid_flag)
    )[: data.shape[0] // 60 * 60]
    per_second_tear = per_second_tear.reshape((-1, 60)).sum(1)

    per_window_tear = per_second_tear[: per_second_tear.shape[0] // 5 * 5].reshape(
        (-1, 5)
    )
    if per_window_tear.size == 0:
        per_window_tear = np.zeros((3, 5))
    per_window_1frame_tear = (per_window_tear > 1).sum(1)
    per_window_3frame_tear = (per_window_tear > 3).sum(1)
    per_window_6frame_tear = (per_window_tear > 6).sum(1)
    ...
    result = [
        ...
        *jitter_buffer.get_buffer_gain(tot_time=tot_time)[
        ...
        avg_render_time,
        avg_proc_time,
        recv_interval,  # avg_dec_time, avg_dec_total_time, avg_render_time, avg_proc_time, recv_interval
        np.sum(
            np.logical_and.reduce((data[:, 11] != -1, tear_flag == 1, cur_valid_flag))
        )
        / tot_time,
        np.sum(
            np.logical_and.reduce((data[:, 11] != -1, tear_flag == 2, cur_valid_flag))
        )
        / tot_time,
        per_second_tear.max(),
        np.percentile(per_second_tear, 95),
        np.percentile(per_second_tear, 80),
        np.percentile(per_second_tear, 50),
        per_second_tear.min(),
        per_second_tear.mean(),
        np.mean(per_second_tear >= 1),
        np.mean(per_second_tear >= 3),
        np.mean(per_second_tear >= 6),
        np.percentile(per_window_1frame_tear, 95),
        np.percentile(per_window_1frame_tear, 80),
        np.percentile(per_window_1frame_tear, 50),
        per_window_1frame_tear.mean(),
        np.percentile(per_window_3frame_tear, 95),
        np.percentile(per_window_3frame_tear, 80),
        np.percentile(per_window_3frame_tear, 50),
        per_window_3frame_tear.mean(),
        np.percentile(per_window_6frame_tear, 95),
        np.percentile(per_window_6frame_tear, 80),
        np.percentile(per_window_6frame_tear, 50),
        per_window_6frame_tear.mean(),
    ]  # teared frame per second,
    # output_file = open('result_bufSize%d_procTime%d_jitterBuffer%d_tsExtrapolator%d_renderTSPredictor%d_renderQueue%d_strict%d_renderCtrlable%d_frameInterval%d.csv' \
    #     %(buf_size, proc_time_estimator_mode, jitter_buffer_controller_mode, timestamp_extrapolator_mode, render_ts_estimator_mode, \
    #     render_queue_mode, strict_buffer, render_ctrlable, frame_interval), 'a')
    # output_file.write(file_path.replace(',', '_') + ', ' + ', '.join([str(item) for item in result]) + '\n')
    # output_file.close()

    # return file_path + ',' + ','.join([" %.3f" %item for item in result])
    for idx in range(1, data.shape[0]):
        if data[idx, 11] == -1:
            action_sample[idx] = -1
        elif display_delayed_flag[idx] == 0:
            action_sample[idx] = 0
        else:
            ts_diff = invoke_present_ts[idx] - frame_ready_ts[idx]
            action_sample[idx] = ts_diff // 17 + 1
        assert action_sample[idx] in [-1, 0, 1, 2]

        lowest_free_slot[idx] = actual_display_slot[idx - 1] + 1

    # path = file_path[:-4] + "_baseline_toptear_buf1_sim.npy"
    # np.save(path, action_sample)
    # path = file_path[:-4] + "_baseline_toptear_buf1_sim_lfs.npy"
    # np.save(path, lowest_free_slot)
...
if __name__ == "__main__":
    final_log_file = open("../log/baseline-alltear-buf2-normal.log", "w")
    for directory in os.listdir("../data"):
        ...
        for file_name in tqdm(os.listdir(os.path.join("../data", directory))):
            ...
            result = cal_single_para_result(
                ...
                max_delay_frame_no=max_delay_frame_no,
                frame_interval=frame_interval,
                render_ts_estimator_mode=render_ts_estimator_mode,
                print_log=True,
            )
            if result[0] is None:
                continue
